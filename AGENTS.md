...
- Keep `release-notes.md` as the versioned user-facing changelog with `## 未发布` at the top. Add only user-visible feature, UI, behavior, compatibility, and bug-fix entries.
- Publish with `pnpm release <patch|minor|major|X.Y.Z>`. The command archives `未发布`, validates, commits, tags, and pushes; release automation extracts only the matching version.
