...
应用使用由外壳单点控制的 8px 外框系统：窗口内容统一使用 8px 内边距，左侧导航和右侧工作区之间统一使用 8px 间隔；展开侧栏宽 228px、折叠为 68px，两块主要区域都以四边 1px 边框和 16px 圆角建立边界，不允许子层再次补偿外框间距。侧栏的非交互区域均可拖动窗口，导航、折叠、主题与未来的表单控件必须显式保持 `no-drag`。窗口使用 40px 无底色自定义拖拽层：macOS 控制点只占用左侧导航上方，右侧配置面板统一内缩 8px；Windows/Linux 控制按钮位于右上角，配置面板保留 40px 顶部安全区。处理页的整个右侧工作区都是拖放表面，配置、文件、待处理列表和任务结果沿同一条 8px 水平基线排列。常规页面使用 20px 水平边距和 24px 分组间距；1100px 以下配置组转为单列，任务表隐藏次要列但保留核心状态和操作。页面切换使用 180ms 交叠淡入淡出，退场层脱离文档流，避免空白帧和内容宽度变化。macOS 使用系统悬浮滚动条，Windows/Linux 在当前 Electron 39 / Chromium 142 上启用 Chromium Overlay 与 Fluent Overlay 滚动条；不得用作者 CSS 覆盖原生滚动条，也不得因页面切换改变内容宽度。
