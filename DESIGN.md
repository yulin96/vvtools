...
应用使用贴合窗口左侧和上下边缘的直边侧栏：展开宽 236px，折叠为 76px，仅以右侧 1px 分隔线连接弹性工作区。窗口使用 40px 无底色自定义拖拽层：macOS 控制点落在左侧导航上方，右侧内容可上移至窗口顶部安全边界；Windows/Linux 控制按钮位于右上角，主内容保留 40px 控制安全区。处理页的整个右侧工作区都是拖放表面，横向外边距保持紧凑，顶部配置面板作为浮起一层的控制区位于同一表面内；文件、待处理列表和任务结果沿稳定纵向流程出现。常规页面使用 20px 水平边距和 22px 分组间距；1100px 以下配置组转为单列，任务表隐藏次要列但保留核心状态和操作。主内容使用透明轨道的悬浮滚动条，出现时不得压缩内容或引起页面切换位移。
